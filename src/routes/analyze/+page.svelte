<script lang="ts">
  ...
  const NullableBooleanParam = createEnumParam(["true", "false", "null"]);

  const filterConfig = {
    locationGermany: BooleanParam,
    locationFinland: BooleanParam,

    cpuCount: NumberParam,
    cpuIntel: BooleanParam,
    cpuAMD: BooleanParam,

    ramInternalSizeLower: NumberParam,
    ramInternalSizeUpper: NumberParam,

    ssdNvmeCountLower: NumberParam,
    ssdNvmeCountUpper: NumberParam,
    ssdNvmeInternalSizeLower: NumberParam,
    ssdNvmeInternalSizeUpper: NumberParam,

    ssdSataCountLower: NumberParam,
    ssdSataCountUpper: NumberParam,
    ssdSataInternalSizeLower: NumberParam,
    ssdSataInternalSizeUpper: NumberParam,

    hddCountLower: NumberParam,
    hddCountUpper: NumberParam,
    hddInternalSizeLower: NumberParam,
    hddInternalSizeUpper: NumberParam,

    selectedDatacenters: ArrayParam,
    selectedCpuModels: ArrayParam,

    extrasECC: NullableBooleanParam,
    extrasINIC: NullableBooleanParam,
    extrasHWR: BooleanParam,
    extrasGPU: BooleanParam,
  };
  ...
  onMount(() => {
    const parsed = decodeQueryParams(
      filterConfig,
  ...
  $: {
    const queryParams = encodeQueryParams(filterConfig, filter);
...
{#if !Number.isNaN(progress) && progress < 100}
  <div class="flex min-h-screen items-center justify-center">
    <div class="w-[40vw]">
      <Progressbar {progress} labelOutside="Loading data…" />
    </div>
  </div>
{:else}
  <div
