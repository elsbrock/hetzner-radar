...
export const GET: RequestHandler = async ({ params, locals, platform }) => {
  ...
  try {
    const alert = await getCloudAlertById(
      platform.env.DB,
...
export const PATCH: RequestHandler = async ({
  ...
  try {
    ...
    const existingAlert = await getCloudAlertById(
      platform.env.DB,
      locals.user.id.toString(),
      params.alertId,
    );
    ...
    if (updates.serverTypeIds !== undefined) {
      if (
        !Array.isArray(updates.serverTypeIds) ||
        updates.serverTypeIds.length === 0
      ) {
        return json(
          { error: "At least one server type must be selected" },
          { status: 400 },
        );
      }
      if (
        !updates.serverTypeIds.every((id: unknown) => typeof id === "number")
      ) {
    ...
    if (updates.locationIds !== undefined) {
      if (
        !Array.isArray(updates.locationIds) ||
        updates.locationIds.length === 0
      ) {
        return json(
