...
export const POST: RequestHandler = async ({ request, platform }) => {
    ...
    try {
        const { changes } = await request.json() as { changes: AvailabilityChange[] };
...
function formatEmailNotification(notifications: Array<{
    alertName: string;
    change: AvailabilityChange;
}>): string {
...
function formatDiscordNotification(notifications: Array<{
    alertName: string;
    change: AvailabilityChange;
}>): string {
