...
export async function getCheapestNvmeConfigurations(conn: AsyncDuckDBConnection): Promise<ServerConfiguration[]> {
	return (await getData<ServerConfiguration>(conn, SQL`
		WITH latest_configs AS (
				SELECT DISTINCT ON (cpu)
						-- Exclude specified columns
...
export async function getCheapestSataConfigurations(conn: AsyncDuckDBConnection): Promise<ServerConfiguration[]> {
	return (await getData<ServerConfiguration>(conn, SQL`
		WITH latest_configs AS (
				SELECT DISTINCT ON (cpu)
						-- Exclude specified columns
