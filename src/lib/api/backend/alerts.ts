...
export async function getAlertsForUser(db: DB, userId: string): Promise<UserAlerts> {
	const activeResultsRaw = await db
		.prepare('SELECT * FROM price_alert WHERE user_id = ? ORDER BY created_at DESC')
		.bind(userId)
		.all();
	...
	const triggeredResults: PriceAlertHistory[] =
		triggeredResultsRaw.results.map(parsePriceAlertHistory);
