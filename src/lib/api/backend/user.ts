...
export async function getUser(db: any, userId: string): Promise<User | null> {
    const result = await db
        .prepare("SELECT id, email, discord_webhook_url, notification_preferences, created_at FROM user WHERE id = ?")
        .bind(userId)
        .first();
...
export async function updateUserNotificationPreferences(db: any, userId: string, preferences: UserNotificationPreferences) {
    await db
