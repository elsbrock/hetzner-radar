...
export async function getCloudAlertsForUser(
  db: DB,
  userId: string,
): Promise<UserCloudAlerts> {
  ...
  const triggeredAlerts: CloudAlertHistory[] = triggeredAlertsRaw.results.map(
    parseCloudAlertHistory,
  );
...
export async function isBelowMaxCloudAlerts(
  db: DB,
  userId: string,
): Promise<boolean> {
  const result = await db
...
export async function deleteCloudAlert(
  db: DB,
  userId: string,
  alertId: string,
): Promise<void> {
...
export async function getActiveCloudAlertsForMatching(
  db: DB,
