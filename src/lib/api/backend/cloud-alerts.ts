...
export async function getCloudAlertsForUser(db: DB, userId: string): Promise<UserCloudAlerts> {
	const activeAlertsRaw = await db
		.prepare('SELECT * FROM cloud_availability_alert WHERE user_id = ? ORDER BY created_at DESC')
		.bind(userId)
		.all();
	...
	const triggeredAlerts: CloudAlertHistory[] =
		triggeredAlertsRaw.results.map(parseCloudAlertHistory);
...
export async function getCloudAlertById(
	db: DB,
	userId: string,
	alertId: string
): Promise<CloudAvailabilityAlert | null> {
	const result = await db
		.prepare('SELECT * FROM cloud_availability_alert WHERE id = ? AND user_id = ?')
		.bind(alertId, userId)
		.first();
...
export async function isBelowMaxCloudAlerts(db: DB, userId: string): Promise<boolean> {
	const result = await db
		.prepare('SELECT COUNT(*) as count FROM cloud_availability_alert WHERE user_id = ?')
		.bind(userId)
		.first<{ count: number }>();
...
export async function deleteCloudAlert(db: DB, userId: string, alertId: string): Promise<void> {
	await db
		.prepare('DELETE FROM cloud_availability_alert WHERE id = ? AND user_id = ?')
		.bind(alertId, userId)
		.run();
}
