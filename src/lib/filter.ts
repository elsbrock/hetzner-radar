...
export function convertServerConfigurationToFilter(
    ...
    return {
        ...
        hddCount: Array(2)
            ...
            .flat()
            .slice(0, 2) as [number, number],
        hddInternalSize: computeFilterRange(
...
export function getHetznerLink(device: ServerConfiguration) {
    const minDriveLength = Math.min(
        device.nvme_size
            ? device.nvme_size / device.nvme_drives.length
            : Infinity,
        device.sata_size
            ? device.sata_size / device.sata_drives.length
            : Infinity,
    ...
    if (minDriveLength < Infinity) {
        filterQ.push(
            `drives_size_from=${Math.floor(minDriveLength / 500) * 500}`,
        );
    }
    if (maxDriveLength > 0) {
        filterQ.push(
