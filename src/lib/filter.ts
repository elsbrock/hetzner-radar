...
export function convertServerConfigurationToFilter(
	...
	return {
		...
		ssdNvmeCount: Array(2).fill(serverConfiguration.nvme_drives.length).flat().slice(0, 2) as [
			number,
			number
		],
		ssdNvmeInternalSize: computeFilterRange(serverConfiguration.nvme_drives, 250),

		ssdSataCount: Array(2).fill(serverConfiguration.sata_drives.length).flat().slice(0, 2) as [
			number,
			number
		],
		ssdSataInternalSize: computeFilterRange(serverConfiguration.sata_drives, 250),

		hddCount: Array(2).fill(serverConfiguration.hdd_drives.length).flat().slice(0, 2) as [
			number,
			number
		],
