<script lang="ts">
	...
	let selectedCurrencyCode = $settingsStore.currencySelection?.code || 'EUR';

	// Helper to format the option text
	function formatOptionText(flag: string, symbol: string, code: string): string {
	...
	function handleCurrencyChange() {
		...
		// When USD is selected, automatically set VAT to 0% (NET)
...
<ButtonGroup size="xs">
	...
	>
		<FontAwesomeIcon icon={faDollarSign} class="mr-2" />
	</InputAddon>
	<Select
		id="currency-select"
		...
		items={Object.entries(CURRENCY_CONFIG).map(([code, option]) => ({
			value: code,
			name: formatOptionText(option.flag, option.symbol, option.code)
		}))}
		size="sm"
		class="w-28 !rounded-s-none bg-white text-xs dark:border-gray-600 dark:bg-gray-700 dark:text-white"
	/>
</ButtonGroup>