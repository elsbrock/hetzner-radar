...
<div
	class={`group relative col-span-8 grid grid-cols-subgrid items-center overflow-hidden rounded-lg bg-white px-4 py-2 text-left ring-1 transition-all duration-150 dark:bg-gray-800
		${
			isBestDeal
				? 'shadow-[0_1px_2px_rgba(234,88,12,0.06)] ring-amber-300/70 hover:ring-amber-400 dark:ring-amber-500/40'
				: 'ring-gray-200/80 hover:ring-gray-300 dark:ring-gray-700/60'
		}
		${clickable ? 'hover:cursor-pointer hover:bg-gray-50/80 focus-visible:ring-2 focus-visible:ring-primary-500/60 focus-visible:outline-none dark:hover:bg-gray-700/40' : ''}`}
...
>
	...
	<div class="min-w-0">
		<div class="flex items-center gap-1.5">
			<span
				class="font-mono text-[13px] leading-tight font-semibold tracking-tight whitespace-nowrap text-gray-900 dark:text-gray-50"
				title={config.cpu}
			>
				{config.cpu}
			</span>
		...
		<div class="mt-0.5 flex items-center gap-1.5 truncate text-[10px] leading-tight text-gray-400 dark:text-gray-400">
	...
	<div class="text-right">
		<div
			class="flex items-center justify-end gap-1 font-mono text-[13px] leading-tight font-semibold tabular-nums whitespace-nowrap text-gray-900 dark:text-gray-100"
		>
			<FontAwesomeIcon icon={faMicrochip} class="h-2.5 w-2.5 text-gray-300 dark:text-gray-500" />
			{config.cpu_cores ?? '?'}<span class="text-gray-300 dark:text-gray-500">/</span>{config.cpu_threads ?? '?'}
		</div>
		<div class="mt-0.5 text-[10px] leading-tight text-gray-400 dark:text-gray-400">cores</div>
	</div>
	...
	<div class="text-right">
		<div
			class="flex items-center justify-end gap-1 font-mono text-[13px] leading-tight font-semibold tabular-nums whitespace-nowrap text-gray-900 dark:text-gray-100"
		>
			<FontAwesomeIcon icon={faMemory} class="h-2.5 w-2.5 text-gray-300 dark:text-gray-500" />
			{config.ram_size}<span class="text-[11px] font-medium text-gray-400 dark:text-gray-400">GB</span>
		</div>
		<div class="mt-0.5 text-[10px] leading-tight text-gray-400 dark:text-gray-400">memory</div>
	</div>

	<!-- 5: Storage (truncates if long) -->
	<div class="min-w-0 text-right">
		<div
			class="flex items-center justify-end gap-1 truncate font-mono text-[13px] leading-tight font-semibold tabular-nums text-gray-900 dark:text-gray-100"
		>
			<FontAwesomeIcon icon={storageIcon} class="h-2.5 w-2.5 shrink-0 text-gray-300 dark:text-gray-500" />
		...
		<div class="mt-0.5 truncate text-[10px] leading-tight text-gray-400 dark:text-gray-400">
	...
	<div class="text-right">
		<div
			class="flex items-center justify-end gap-1 font-mono text-[13px] leading-tight font-semibold tabular-nums whitespace-nowrap text-gray-900 dark:text-gray-100"
		>
			<FontAwesomeIcon icon={faGaugeHigh} class="h-2.5 w-2.5 text-gray-300 dark:text-gray-500" />
		...
		<div class="mt-0.5 text-[10px] leading-tight text-gray-400 dark:text-gray-400">gb6 sc/mc</div>
	</div>
	...
	<div>
		<div class="flex h-[16px] items-center gap-1">
			{#each extras as ext (ext.label)}
				<span
					class="inline-flex items-center gap-0.5 rounded bg-gray-100 px-1 py-0 font-mono text-[10px] leading-none font-medium text-gray-600 dark:bg-gray-700 dark:text-gray-300"
					title={ext.title}
				>
					<FontAwesomeIcon icon={ext.icon} class="h-2 w-2 text-gray-400 dark:text-gray-400" />
					{ext.label}
				</span>
			{/each}
		</div>
		<div class="mt-0.5 text-[10px] leading-tight text-gray-400 dark:text-gray-400">extras</div>
	</div>
	...
	<div class="text-right">
		<div class="flex items-baseline justify-end gap-0.5 leading-tight">
			<span
				class="font-mono text-[13px] font-semibold tabular-nums text-gray-900 dark:text-gray-50"
			>
			...
			<span class="text-[11px] font-medium text-gray-500 dark:text-gray-400">{$currencySymbol}</span>
		</div>
		<div class="mt-0.5 text-[10px] leading-tight text-gray-400 dark:text-gray-400">
