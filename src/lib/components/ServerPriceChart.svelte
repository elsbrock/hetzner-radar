<script lang="ts">
  ...
  let vatSuffix = $derived(
    selectedOption.rate > 0
      ? `(incl. ${(selectedOption.rate * 100).toFixed(0)}% VAT)`
      : "(net)"
  );
  ...
  $effect(() => {
    ...
    const priceBackgroundColor = isDarkMode
      ? "rgba(251, 146, 60, 0.1)"
      : "rgba(249, 115, 22, 0.1)";
            ...
            label: function (context: TooltipItem<any>) {
              ...
              if (context.parsed.y !== null) {
                ...
                if (context.dataset.label === "Price") {
                  ...
                  const tooltipDecimalPlaces =
                    timeUnitPrice === "perHour" ? 4 : 2;
...
<div class="relative h-full w-full" data-testid="server-pricechart">
  ...
  <div
    class="h-full w-full {loading || noResults
