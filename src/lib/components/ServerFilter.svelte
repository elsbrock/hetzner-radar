<script lang="ts">
    ...
    if ($filterStore === null) {
        console.log(
            "ServerFilter: Initializing filter store with default values",
        );
    ...
    $effect(() => {
        ...
        if (currentFilterState !== previousFilterState) {
            console.log(
                "ServerFilter: Filter changed, updating store with:",
                filter,
            );
            console.log(
                "ServerFilter: Previous filterStore value:",
                $filterStore,
            );
    ...
    $effect(() => {
        ...
        if (filterString) {
            ...
            if (JSON.stringify(decodedFilter) !== JSON.stringify(filter)) {
                console.log(
                    "ServerFilter: Updating filter from URL with:",
                    decodedFilter,
                );
    ...
    $effect(() => {
        ...
        ssdNvmeSizeLower = getFormattedDiskSize(
            filter.ssdNvmeInternalSize[0],
            500,
        );
        ssdNvmeSizeUpper = getFormattedDiskSize(
            filter.ssdNvmeInternalSize[1],
            500,
        );
        ssdSataSizeLower = getFormattedDiskSize(
            filter.ssdSataInternalSize[0],
            500,
        );
        ssdSataSizeUpper = getFormattedDiskSize(
            filter.ssdSataInternalSize[1],
            500,
        );
...
<div class="flex flex-col w-full sm:w-auto">
    ...
    <div
        class="flex items-center justify-between py-2 {isFilterCollapsed
            ? 'sm:hidden'
            : ''}"
    >
    ...
    {#if isFilterCollapsed}
        <div class="hidden sm:flex flex-col items-center py-2 mt-4">
            <Button
                color="alternative"
                size="sm"
                class="!p-2 mb-4"
            ...
            <span
                class="vertical-text text-lg font-semibold text-gray-700 dark:text-gray-300"
    ...
    <ul
        class="space-y-2 font-medium {isFilterCollapsed ? 'hidden' : ''}"
