<script lang="ts">
    ...
    interface $$Props {
        datacenters: NameValuePair[];
        cpuModels: NameValuePair[];
        isFilterCollapsed?: boolean;
    }
    // Make collapsed state bindable for the parent
    let { datacenters, cpuModels, isFilterCollapsed = $bindable(false) } = $props();
...
<div class="flex flex-col w-full sm:w-auto">
    ...
    <div class="hidden sm:flex flex-col items-center py-2 mt-4">
        ...
        <span class="vertical-text text-lg font-semibold text-gray-700 dark:text-gray-300">Filter Settings</span>
    </div>
    {/if}
