...
test.describe('Server Interactions', () => {
  test('should open server detail drawer when clicking server card', async ({ page }) => {
  ...
  test('should display server details in drawer', async ({ page }) => {
    ...
    await expect(drawer.getByRole('heading', { name: 'Auctions', exact: true })).toBeVisible();
  ...
  test('should display auctions table in drawer', async ({ page }) => {
    ...
    const noAuctions = await drawer.getByText('No matching auctions found').isVisible();
  ...
  test('should show price chart in drawer', async ({ page }) => {
    ...
    await expect(drawer.locator('text=/\\d+\\.\\d{2} €/').first()).toBeVisible();
  ...
  test('should have clickable external links in drawer', async ({ page }) => {
    ...
    const searchButton = drawer.locator('button').filter({ hasText: /external/i }).or(
      drawer.locator('a[href*="hetzner"]')
    );
