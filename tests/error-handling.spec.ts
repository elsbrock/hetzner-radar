...
test.describe('Error Handling', () => {
  ...
  test('should handle slow network conditions gracefully', async ({ page }) => {
    ...
    const loadingElements = page.locator('text=/loading|spinner|downloading/i').or(
  ...
  test('should handle navigation to invalid analyze parameters', async ({ page }) => {
    ...
    await expect(page.getByTestId('server-filter')).toBeVisible({ timeout: 10000 });
  ...
  test('should handle rapid user interactions without crashing', async ({ page }) => {
    ...
    if (await germanyFilter.isVisible() && await netherlandsFilter.isVisible()) {
  ...
  test('should handle browser back/forward navigation properly', async ({ page }) => {
    ...
    await expect(page.getByText(/configurations|server.*config/i).first()).toBeVisible();
    ...
    await expect(page.getByTestId('server-filter')).toBeVisible({ timeout: 15000 });
  });

  test('should handle large dataset filtering without performance issues', async ({ page }) => {
    await page.goto('/analyze');
    ...
    // Get initial count
    const initialText = await page.getByTestId('total-configurations').textContent();
    ...
    if (initialCount > 50) {
      ...
      const filteredText = await page.getByTestId('total-configurations').textContent();
