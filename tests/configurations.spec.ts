...
test.describe('Configurations Page (/configurations)', () => {
  ...
  test('should display the main heading and introduction', async ({ page }) => {
    await expect(
      page.getByRole('heading', { name: 'Choose the Right Server for Your Needs', level: 1 })
    ).toBeVisible();
  ...
  test('should display configuration sections and server cards after loading', async ({ page }) => {
    ...
    const affordableSection = page.locator('div:has(h2:has-text("Most Affordable Configurations"))');
    // Wait for text content ("seen") inside the first card to appear, confirming data load.
    await expect(affordableSection.locator('text=/seen/').first()).toBeVisible({ timeout: 15000 });
    ...
    await expect(page.getByRole('heading', { name: 'Most Affordable Configurations' })).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: 'High-Performance NVMe Storage' })).toBeVisible();
    ...
    await expect(page.locator('div:has(h2:has-text("Best Value for Disk Space"))').locator('> div > div:has(h5)').first()).toBeVisible();
    await expect(page.locator('div:has(h2:has-text("Best Value for Memory"))').locator('> div > div:has(h5)').first()).toBeVisible();
    await expect(page.locator('div:has(h2:has-text("High-Performance NVMe Storage"))').locator('> div > div:has(h5)').first()).toBeVisible();
    await expect(page.locator('div:has(h2:has-text("Affordable SATA SSDs"))').locator('> div > div:has(h5)').first()).toBeVisible();
  });
  ...
  test('should display the call to action section', async ({ page }) => {
    ...
    const analyzeButton = page.locator('section:has(h2:has-text("Ready to Explore More?"))')
