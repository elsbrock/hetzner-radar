...
test.describe('Advanced Filtering', () => {
  test('should combine multiple filters effectively', async ({ page }) => {
    ...
    const afterLocationText = await page.getByTestId('total-configurations').textContent();
    const afterLocationCount = parseInt(afterLocationText?.match(/(\d+)/)?.[1] || '0');
    
    // Apply memory filter (assume there's a memory range slider or input)
    const memoryFilter = page.getByLabel(/memory/i).or(page.getByText(/32.*GB/)).first();
  ...
  test('should persist filter state after page refresh', async ({ page }) => {
    ...
    const afterReloadText = await page.getByTestId('total-configurations').textContent();
    const afterReloadCount = parseInt(afterReloadText?.match(/(\d+)/)?.[1] || '0');
    ...
    if (!isChecked) {
      console.log('Filter persistence may not be working - checkbox not checked after reload');
    }
  });
  ...
  test('should clear all filters when using clear button', async ({ page }) => {
    ...
    const clearedResultsText = await page.getByTestId('total-configurations').textContent();
    const clearedCount = parseInt(clearedResultsText?.match(/(\d+)/)?.[1] || '0');
  ...
  test('should handle price range filtering', async ({ page }) => {
    ...
    const priceControls = page.getByText(/price/i).or(page.locator('[data-testid*="price"]'));
    ...
    if (await priceControls.first().isVisible()) {
      ...
      if (await priceSlider.isVisible()) {
        ...
        const resultText = await page.getByTestId('total-configurations').textContent();
  ...
  test('should handle edge case with no results', async ({ page }) => {
    ...
    if (await priceSlider.isVisible()) {
      ...
      if (await netherlandsFilter.isVisible() && await finlandFilter.isVisible()) {
    ...
    if (count === 0) {
      ...
      const noResultsMessage = page.getByText(/no.*result/i).or(page.getByText(/no.*configuration/i));
  ...
  test('should maintain sorting when filters change', async ({ page }) => {
    ...
    const sortControls = page.getByText(/sort/i).or(page.locator('[data-testid*="sort"]'));
    
    if (await sortControls.first().isVisible()) {
      // Try to change sorting (e.g., to price high to low)
      const sortButton = page.getByRole('button').filter({ hasText: /sort/i }).first();
    ...
    if (cardCount > 1) {
      const firstCardPrice = await serverCards.first().locator('text=/\\d+\\.\\d{2} €/').first().textContent();
      const secondCardPrice = await serverCards.nth(1).locator('text=/\\d+\\.\\d{2} €/').first().textContent();
