...
test.describe("Analyze Page Tests", () => {
  test("should load analyze page with basic functionality", async ({
    page,
  }) => {
    ...
    await page
      .waitForSelector('[data-testid="server-card"]', { timeout: 15000 })
      .catch(() => {
    ...
    if (await popularityStat.isVisible({ timeout: 5000 })) {
      ...
      await expect(
        popularityStat.locator("text=Server Popularity"),
      ).toBeVisible();
